...
def DirtyIMC(X, Y, U_init, V_init, W_init, H_init,P_init,Q_init,fea_loc, label_loc, alpha, lambda0, lambda1,
...
# cost-sensitive
def DirtyIMC3(X, Y, U_init, V_init, W_init, H_init,P_init,Q_init,G_init, fea_loc, label_loc, alpha, lambda0, lambda1,
    ...
    with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
      ...
      ##### regularization
        ...
        H_frobenius_norm = tf.reduce_mean(tf.pow(H, 2, name='H_frobenius'))
        norm_sums_label_M = tf.add(G_frobenius_norm, H_frobenius_norm, name='norm_reg_label_M')
        regularizer_label_M = tf.multiply(norm_sums_label_M, lambda3, 'regularizer_label')
        cost_2 = tf.add(cost_1, regularizer_label_M)
        diff_XWH = tf.subtract(tf.add(tf.matmul(S,B),N), M, name='diff_XWH')
        #### L2-norm
...
if __name__ == '__main__':
    ...
    # DirtyIMC 方法进行初始化，N为论文中其他信息;W/H用于side information;P/Q 用于N
