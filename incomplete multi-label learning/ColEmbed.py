...
def ColEmbed_L(X, Y, U_init, V_init, W_init, H_init, B_init, fea_loc, label_loc, alpha, lambda0, lambda1,
...
def ColEmbed_NL(X, Y, U_init, V_init, W_init, H_init, B_init, ffproj, fea_loc, label_loc, alpha,
                                     lambda0, lambda1, lambda2, lambda3, delta, kx, lr, max_iter):
    alpha_weight = tf.constant((1. - alpha), name='alpha', dtype=tf.float64)  ### 1 - alpha
    beta_weight = tf.constant((2. * alpha - 1), name='beta', dtype=tf.float64)  ### 2* alpha -1
    fea_loc_x = fea_loc[0]
    fea_loc_y = fea_loc[1]
...
if __name__ == '__main__':
    ...
    U,V,W,H,B = ColEmbed_L(train_data,train_label_masked,U_init,V_init,W_init,H_init,B_init,fea_loc,label_loc,
    ...
    #                                     alpha,lambda0,lambda1,lambda2,lambda3,delta,nrank,5,500)
